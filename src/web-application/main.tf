...
resource "azurerm_service_plan" "eshop_identity_web_plan" {
  name                = "${var.environment}-eshop-identity-web-plan"
  location            = var.location
  resource_group_name = azurerm_resource_group.webapp_rg.name
  os_type             = "Windows"
  sku_name            = var.eshopwebapp_identity_plan_tier
}

resource "azurerm_windows_web_app" "eshop_identity_web_app" {
  ...
  location            = var.location
  resource_group_name = azurerm_resource_group.webapp_rg.name
  service_plan_id     = azurerm_service_plan.eshop_identity_web_plan.id

  site_config {
    application_stack {
      dotnet_version = "v7.0"
    }
    always_on = var.eshopwebapp_identity_plan_tier == "D1" ? false : true
  }
  ...
  }, var.eshop_identity_web_app_settings)
}